...
@pytestmark
async def test_get_news_tags_by_id(client: AsyncClient) -> None:
    test_data = [
        {
            "id": 3,
            "name": "Địa lí"
    ...
    response = await client.get("api/news/1/tags/")
    assert response.status_code == HTTP_200_OK
    assert response.json() == test_data

@pytestmark
async def test_get_news_tags_by_id_not_exist(client: AsyncClient) -> None:
